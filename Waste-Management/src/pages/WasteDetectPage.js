...
const WasteDetectPage = () => {
  ...
  useEffect(() => {
    fetch('http://127.0.0.1:5050/api/screenshots')
  ...
  const handleDelete = (screenshot) => {
    ...
    fetch(`http://127.0.0.1:5050/api/screenshots/${filename}`, {
      method: 'DELETE',
    })
      .then((response) => {
        if (response.ok) {
      ...
      <div className="image-grid">
        {filteredScreenshots.map((screenshot, index) => (
          <div key={index} className="image-wrapper">
            <img
              src={`http://127.0.0.1:5050${screenshot}`}
              alt={`Screenshot ${index + 1}`}
              className="image-thumbnail"
              onClick={() => setEnlargedImage(`http://127.0.0.1:5050${screenshot}`)}
            />
