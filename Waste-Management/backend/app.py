...
def add_dustbin():
    try:
        ...
        if not location or b_value is None or nb_value is None:
            return jsonify({"error": "Missing data. Ensure 'location', 'bValue', and 'nbValue' are provided."}), 400

        # Insert into MongoDB
        dustbins_collection.insert_one({
            "location": location,
            "bValue": int(b_value),  # Convert to integer
            "nbValue": int(nb_value)
        })
...
def delete_dustbin():
    try:
        ...
        if not location:
            return jsonify({"error": "Location is required to delete a dustbin"}), 400
...
def update_dustbin():
    try:
        ...
        result = dustbins_collection.update_one(
            {"location": location},
            {"$set": {"bValue": int(b_value), "nbValue": int(nb_value)}}
        )
